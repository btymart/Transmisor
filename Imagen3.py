...
class Imagen:
    ...
    def generarImagen(self):  
        ...
        for x in range(colReferencia.size):
            ##SUPERIOR
            pt1 = (tamanoCelda*(((x+1))+1+1) , (2)*tamanoCelda)
            pt2 = (tamanoCelda*(((x+1))+2+1) , (3)*tamanoCelda)
            col = self.Color(colReferencia[x])
            cv2.rectangle(img,pt1,pt2,col,-1)

            ##DERECHA
            pt1 = ((5+self.tamanoMatriz)*tamanoCelda,tamanoCelda*(((x+1))+1+1))
            pt2 = ((6+self.tamanoMatriz)*tamanoCelda,tamanoCelda*(((x+1))+2+1))
            col = self.Color(colReferencia[x])
            cv2.rectangle(img,pt1,pt2,col,-1)

            ##INFERIOR
            pt1 = (tamanoCelda*((((x+1))) + (self.tamanoMatriz+5-(colReferencia.size))),(5+self.tamanoMatriz)*tamanoCelda)
            pt2 = (tamanoCelda*((((x+1))) + (self.tamanoMatriz+4-(colReferencia.size))),(6+self.tamanoMatriz)*tamanoCelda)
            col = self.Color(colReferencia[x])
            cv2.rectangle(img,pt1,pt2,col,-1)
